function [summ] = SaCD_bounded_slow_changing_trajectories_SMCEEM(parameters,args)
...
%     solution = args.solver(@(t,n) lotkaVolterra(t,n,A,r), time_array, IC);
    ...
    % partial solutions cannot be treated as full ones
...
%% Measure discrepancy
%look at properties of trajectory
if error ==false
    ...
    relative_change_per_time = zeros(args.n_species,size(solution.x,2)-1);
    for i=1:length(solution.x)-1
        relative_change_per_time(:,i) = (solution.y(:,i+1)-solution.y(:,i))./(solution.y(:,i)*(solution.x(i+1)-solution.x(i)));
    end

    %calculate relative change above max as area
    rates_above = max(relative_change_per_time - args.R_max,0);
    for sp = 1:args.n_species
        R_area_above(sp) = trapz(solution.x(2:end), rates_above(sp,:));
    end

    %calculate relative change below min as area
    rates_below = max(args.R_min - relative_change_per_time,0);
    for sp = 1:args.n_species
        R_area_below(sp) = trapz(solution.x(2:end), rates_below(sp,:));
    end
    ...
    summ = measured_population_deficiency+ measured_population_surplus+measured_population_spikes+measured_population_drops;
