function[fillhandle,msg]=jbfill_withLinestyle(xpoints,upper,lower,color,edge,add,transparency,lineStyle)
% John Bockstege (2023). Shade area between two curves 
...
if length(upper)==length(lower) && length(lower)==length(xpoints)
    ...
    set(fillhandle,'EdgeColor',edge,'FaceAlpha',transparency);%set edge color
    set(fillhandle,'LineStyle',lineStyle,'LineWidth',1);%set dashed edges

    %removefromlegend(fillhandle);
%     if add
%         hold off
%     end
else
